...
gi::dfa* gi::blueStar::run(string base_path)
{
	...
	{		
		...
		{	
			...
			for(int j=0; j<n_red; ++j){
				...
				// TODO AGGIUNGI LA SET ACCEPTOR OR REJECTOR STATES
	...
	cout << "Numero di stati: "<< finalDFA->get_num_states() << endl;

	idsm::compute_ir_stats(finalDFA, ir_stats_after, positive, dim_positive, negative, dim_negative, wp, wn);
	...
	for(int i=0; i<dim_negative; ++i)
		if(!finalDFA->membership_query(negative[i])){
			cout << "ENTRATO IN CORRETTAMENTE" << endl;
			cout << "WN: "<<wn[i] << endl;
			countN += wn[i];
		}
	...
	cout << "MINIMZZATO HA STATI: "<< finalDFAmin->get_num_states() << endl;
