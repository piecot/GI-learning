...
gi::dfa* gi::lstar::run(bool approximate, string samplestestpath){
    ...
    #ifdef DEBUG_1
       obtable->experiment->setNumState(this->experiment->getNumState());
       obtable->experiment->setType("lstar");
    #endif
	...
	while(1){
		...
		dfahp = obtable->obt_to_dfa();

		#ifdef DEBUG_2
		dfahp->print_dfa_ttable("DFA_HP dalla TABELLA DI OSSERVAZIONE");
		#endif
		...
		vector<SYMBOL> witness;
		 if(!approximate)
		 {
			...
			#ifdef DEBUG_1
				obtable->experiment->set_n_eq_query();
			#endif
		}
		  else
		...
		if(0 != witness.size()){
			        #ifdef DEBUG_1
			            obtable->experiment->setSizeCounterexamples(witness.size());
			        #endif
					obtable->update_from_counterexample(witness);
		}else{
    ...
    #ifdef DEBUG_1
       obtable->experiment->myPrint("../esperimenti/experiments.txt");
    #endif
    delete obtable;
    
    //dfa dfaInferred(*dfahp);
